...
| To the Right | Check to display the legend to the right side. Unchecked by default.
| Width | Available when *To the Right* is checked. Sets the minimum width (in pixels) for the legend. Leave it blank to allow for the automatic width selection.
|===
