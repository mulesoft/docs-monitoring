...
Configure time-series data in a graph. A custom dashboard graph in Anypoint Monitoring displays a metric for a Mule app or API.

Advanced options support fine-tuned configurations. You can also customize X and Y axes, the look of the graph, legends, and behavior of the graph when you hover over a data point.
...
* <<visuals, *Visuals*>>: Style settings, including bar, line, or data point settings
...
* <<axes, *Axes*>>: Configure your graph's axes.
...
Create or navigate to the graph you plan to configure:

* If your graph exists, open its configuration view:
...
+
The description becomes available from an info bubble on the graph.
. Select and configure a graph mode (basic or advanced).
+
For more information, see <<general>>.
. Set the graph's axes or use the defaults.
+
For more information, see <<axes>>.
. Configure <<visuals, bars, lines, or points>> for the graph.
. Optionally:
.. Configure an <<alerts_config, alert and email notification>> for a _basic mode_ graph.
.. Add <<advanced, advanced configurations>>. 
. Click *Apply Changes*. 
