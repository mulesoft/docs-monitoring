...
To open a configuration page for a graph, see xref:dashboard-custom-config.adoc[Configuring Custom Dashboards].
...
| As Table | Checking displays the selected legend Values (such as, Min and Max) in a table. Unchecked by default.
| To the Right | Checking display the legend to the right side. Unchecked by default.
| Width | Available when To the Right is checked. Sets the minimum width (in pixels) for the legend. You can leave it blank to allow for the automatic width selection.
|===
...
Note that thresholds are not available when an alert is in place on the graph. To re-enable thresholds for a chart, you must delete the alert.
...
| Threshold type | Identifier for the threshold. One of Custom, Critical, Warning, or OK.
