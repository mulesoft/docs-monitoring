...
Configure groups of time-series data in a graph. A custom dashboard graph in Anypoint Monitoring displays a metric for a Mule app or API in a given environment, such as your Design or Sandbox environment.

Advanced configuration options support fine-tuned data sets. You can also customize the X and Y axes, the look of the graph, legends, and behavior when you hover over a data point.
...
* To open the graph's configuration view, click its image:more-menu-vert-ellipsis.png["",15,15] (*More menu*), and select *Configure*.
