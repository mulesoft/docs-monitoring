...
:description: Configure table charts in MuleSoft Anypoint Monitoring custom dashboards to display detailed metric data for Mule apps and APIs in tabular format.
:keywords: MuleSoft, Anypoint Monitoring, custom dashboards, table chart, tabular metrics, dashboard visualization, Mule app metrics, API metrics

Configure tables with Mule app and API metrics in Anypoint Monitoring custom dashboards. 

image::dashboard-custom-config-table.png["Table chart and configuration view"]

[calloutlist]
. A table configured with multiple queries in advanced mode
...
. Optionally:
.. In the table's <<time-range, *Time range*>> tab, set the table's overrides for the time range selected by users.
