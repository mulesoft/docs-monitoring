...
The Telemetry Exporter enables you to export xref:access-management::audit-logging.adoc[_audit logs_] to third-party observability and analytics apps, such as Azure Monitor, Datadog, and Splunk HEC.
...
include::monirtoring::partial$telemetry-exporter-notes.adoc[tag=knownIssuesGen]
include::monirtoring::partial$telemetry-exporter-notes.adoc[tag=knownIssuesAL]