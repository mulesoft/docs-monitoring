...
* An Anypoint Platform account with the Telemetry Exporter Administrator and Telemetry Exporter Configuration Manager permissions
...
include::partial$include-nav-steps-telemetry-permissions.adoc[]
...
include::partial$include-nav-steps-telemetry-permissions.adoc.adoc[]
...
The Telemetry Exporter feature runs a service every hour to apply the changes for the configurations and connections, so once the configuration is created, it can take up to an hour for the configuration to be applied and start exporting data. 
