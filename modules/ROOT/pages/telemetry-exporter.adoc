...
The Telemetry Exporter enables you to export xref:access-management::audit-logging.adoc[_audit logs_] and trace data from CloudHub 2.0 apps to third-party observability and analytics apps, such as Azure Monitor, Datadog, and Splunk HEC.
...
* Enabled trace data collection for CloudHub 2.0 apps deployed to xref:hosting::ch2-deploy-shared-space.adoc#enable-trace-data-collection[shared] or xref:hosting::ch2-deploy-private-space.adoc#enable-trace-data-collection[private spaces]. This requirement applies only if you want to export trace data.
