...
* *Request Volume*: The total number of inbound requests to the app within the specified time range.
* *Failed Requests*: The total number of failed inbound requests to the app for the specified time range. An inbound request failure occurs when a Mule flow within the deployed Mule app throws an exception. Inbound failures differ from outbound failures, which indicate errors from outbound connector operations, such as an HTTP error code from an HTTP Request operation in the Anypoint Connector for HTTP (HTTP Connector).
