...
Log tokenization is available for Hybrid and CloudHub deployments running on xref:release-notes::mule-runtime/mule-esb.adoc[supported Mule versions].

== Before You Begin

//TODO: version 1.13 RTF?? Any other versions? Those pages don't exist in latest RTF docs, btw. 
...
. In API Manager, xref:@api-manager::getting-started-proxy.adoc[set up the API gateway] for the Mule app to serve as the proxy.
