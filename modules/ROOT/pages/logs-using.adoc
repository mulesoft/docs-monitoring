...
[NOTE]
For every result group, the *Quick Filters* list displays the first five results of the most recent 500 log entries.
When there are more than 500 entries or the top five values do not represent 100% of total values, a *See More* link for each result group appears.
You can click *See More* to open the *Add a Filter* box and set parameters to filter your results.
The *See More* functionality provides control over the filters you want to apply. *See More* does not allow you to filter the results obtained using *Quick Filters*.
