...
+
Provide full terms, such as `"code 400"` to find  bad requests. For
exact matches to multi-word searches, surround the search term
...
+
Use wildcards such as `payload=org.glassfish.grizzly*` if you don't know the full exact term. 

Example results from a wildcard search:
+
