...
==== Note For On-Premise Servers With Customized Logging

Logs produced by apps with customized logging (by changing the pattern layout in the `log4j2.xml` file) might not get indexed correctly into Anypoint Monitoring. To ensure correct indexing, it is recommended to use Mule default pattern layout, which is one of the following: 

* `<PatternLayout pattern="%-5p %d [%t] [event: %X{correlationId}] %c: %m%n" />`, or
* `<PatternLayout pattern="%d [%t] %-5p %c - %m%n" />`

==== Note For Applications Deployed to Cloudhub and Runtime Fabric With Customized Time Zone
Do not override the default JVM timezone(it should always be UTC). Changing the timezone to a local timezone will cause logs indexed into Anypoint Monitoring with an incorrect timestamp. 
