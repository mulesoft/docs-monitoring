...
. Click *View*. 
+
On the API dashboard, you can view information about a running, managed API. 
. Select the date and time range to view the statistics for and click *Confirm*. 
. To view API metrics in API Manager, click the *View API in API Manager* link.
...
This metric is not supported for APIs deployed through Flex Gateway.
+
NOTE: {note-flex-deployments-not-supported}
