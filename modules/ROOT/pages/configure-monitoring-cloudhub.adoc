...
== Enable Monitoring for Apps Deployed to CloudHub by Default

You can configure Anypoint Monitoring to enable monitoring by default for any newly deployed apps to CloudHub. When you enable the *(checkbox name)*, any Mule app you deploy to CloudHub has monitoring enabled by default.

This feature is available only if you have a Titanium subscription to Anypoint Platform, and you must use one of the following Mule runtime versions:

* 3.88 or later
* 3.9.x
* 4.x
// Check proper articulation for these versions. This list appears in the built-in dashboards UI.
If you have a Titanium subscription to Anypoint Platform, this feature is enabled by default, and monitoring is enabled for all newly deployed Mule apps.
