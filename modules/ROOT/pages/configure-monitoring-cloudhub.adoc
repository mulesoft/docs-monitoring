...
Anypoint Monitoring non-AM images receive patch updates as soon as they are available. Anypoint Monitoring -AM images are no longer receiving patch updates. It is recommended that you switch to a non-AM image if your production environment requires a Mule runtime engine patch.
...
Logs are only available if you have a Titanium subscription to Anypoint Platform.
