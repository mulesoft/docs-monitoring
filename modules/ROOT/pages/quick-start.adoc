...
Anypoint Monitoring provides data on Mule apps that are running on <<runtime_versions>>. It is part of the Management Center in Anypoint Platform, and you need to have the <<required_permission>> to use it.
...
[[runtime_versions]]
== Supported CloudHub Mule Versions

For Anypoint Monitoring to start monitoring your Mule apps, you must deploy them to the appropriate version of Mule runtime engine.  Anypoint Monitoring images (for example, 3.9.1-AM) receive the latest patch updates once per month, while the non-AM (mainline) images receive patch updates as soon as they are available. If your production environment requires a Mule runtime engine patch, you should switch to a mainline runtime.
