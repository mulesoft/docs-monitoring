...
For complete functionality, you can use the following runtime versions with Anypoint Monitoring:

* Mule 3: versions 3.8.7 or newer
* Mule 4: versions 4.1.6 or newer

If you are using a Mule 4 version that is older than version 4.1.5, you cannot use enhanced metrics for API Analytics.

Versions 3.8.6 or older are not supported. Additionally, all runtime versions that have the `-AM` suffix are deprecated.
...
You can xref:am-installing.adoc[install Anypoint Monitoring] on an on-premises server to monitor applications that are running on that server and managed in CloudHub (hybrid apps).
