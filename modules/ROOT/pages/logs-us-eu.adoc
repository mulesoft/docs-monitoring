...
API Manager and Agent Visualizer provide links to logs for agentic assets (agent, broker, or MCP server instances):

* In API Manager, navigate to an agentic API instance that runs on *Runtime* type Flex Gateway:
** To find Flex Gateway logs, click *Logs* from the sidebar, and then click *View logs in Anypoint Monitoring*.
** To find logs policies applied to agents, brokers, and MCP server, click *Message Logs* from the sidebar, and then click *View logs in Anypoint Monitoring*.
** For a broker's reasoning logs, click *Message Logs* from the sidebar, and then click the button dedicated to the broker. These logs come from the broker application.
* In Agent Visualizer, click a node in the visual graph. For guidance, see xref:agent-visualizer.adoc#access-logs[Access Logs] in the Agent Visualizer documentation.
+
For more information about Agent Visualizer, see xref:agent-visualizer::index.adoc[].

[IMPORTANT]
--
By default, broker logs include the actions the broker takes (including agents called and MCP tools executed), but they don't include reasoning data due to privacy concerns.

Enabling reasoning logs risks inclusion of personally identifiable information and secrets. 
--
...
For guidance with this setting for deployments to CloudHub 2, see xref:cloudhub-2::ch2-deploy-shared-space.adoc#configure-logging[Configure Log Levels and Categories].
