= Searching Logs in Anypoint Monitoring (US and EU)
:page-info-pills: anypoint-pricing-titanium

//EXPERIENCE ROLLOUT BANNER FOR THIS PAGE TO REMOVE ONCE ROLLOUT IS COMPLETE
[.notice-banner]
From May through June 2026, Anypoint Monitoring is rolling out an xref:monitoring::logs-search-hf.adoc[enhanced Log Search experience] in US and EU instances of Anypoint Platform. This version replaces the traditional Log Search.
...
* Mule app logs
* Omni Gateway (formerly Flex Gateway) logs
* Agentic asset logs:
...
API Manager also displays Omni Gateway (formerly Flex Gateway), message, and broker logs.
////
...
When you search for logs for APIs deployed through Omni Gateway (formerly Flex Gateway), use the *Application* filter. Use the *App ID* field to identify the instance.
...
NOTE::
Results for the *Application* filter include APIs deployed through Omni Gateway (formerly Flex Gateway).
