...
[[search-agentic]]
== Get Agent, Broker, and MCP Server Logs

API Manager and Agent Visualizer provide links to logs to agentic assets (an agent, broker, or MCP server instance). To search for these logs from Anypoint Monitoring, provide the instance ID for the application or API in the search field with any filters to narrow your query.

* In API Manager, navigate to an agentic API instance that runs on *Runtime* type Flex Gateway, and click *Logs*.
* In Agent Visualizer, click a node, go to *Performance and Health*, and click *View Logs*.
