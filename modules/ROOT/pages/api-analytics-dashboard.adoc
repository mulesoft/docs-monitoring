...
+
This metric is not supported for APIs deployed using the Flex Gateway.
...
The Basic Metrics view is not available for APIs deployed using the Flex Gateway.
...
Titanium users can also enable non-aggregated API events by setting this property: `anypoint.platform.config.analytics.agent.api_raw_metrics.enabled=true`.
