...
| Select a|
Add one or more `field` settings to create a function that selects a field (such as `heap_committed`,`heap_total`, or `heap_used`) from the *From* data (such as `jvm.memory`) and applies an operation to it (such as the `mean` of the `heap_total`). 
