...
[calloutlist]
. Search for log field columns to display or remove from the log *Results* table. From an expanded field column heading, you can:

* View the highest five values in the field
* Add or remove the field from the *Results* table
* Filter by any of the displayed values or, if the field contains more than 5 values, create a <<search-filters, search filter>> from *See More*
+
You can also create a search filter from <<filter-simple, *+ Add filter*>>.
