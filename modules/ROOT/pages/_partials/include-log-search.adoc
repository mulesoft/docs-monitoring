...
For example, assume that a log search query language index contains two fields (`application` and `message`) and that the message is the default field. To find a message that contains "called the function Value Selector" in the app "my-app", enter either of the following:
