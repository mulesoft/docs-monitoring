...
|`AND`     |The `AND` operator matches documents where both terms exist anywhere in the text of a single document. This is equivalent to an intersection using sets. |To search for documents that contain "anypoint monitoring" use the query:
...
For example, to find logs with a message that contains "called the function Value Selector" in the app "my-app", enter either of the following:
...
== Exact-Match Searches for Messages

To search for an exact match to all tex in a _single-line_ log `message` field, you can use the `message.keyword:` prefix followed by a string in double quotatation marks. The string must exactly match all the log message's content. 

Example:

* message.keyword: "System started successfully"
+
This query returns any logs with a message field that matches the entire value exactly, including spaces, punctuation, and case.

The prefix `message.keyword:` makes the search evaluate the full message, including spaces, punctuation, and case.

// tag::regex[]
[[regex]]
== Regular Expressions for Messages

Use a regular expressions with the `message.keyword:` prefix to search for log messages. 

Examples:

* `message.keyword: /Error code: [0-9]+/`
+
This pattern matches any message that contains `Error code:` followed by a space and one or more digits ([0-9]+), such as `Error code: 12345`.
* `message.keyword: /Stop.*/`
+
The pattern matches any message that starts with `Stop` followed by zero or more characters (`.*`), such as `Stop processing` or `Stopping DefaultSchedulerService...`.
* `message.keyword: /.*java.io.*/`
+
This pattern matches any message that contains the substring `java.io`, such as `java.io.FileNotFoundException` or `Error in java.io stream`.
* 
// end::regex[]