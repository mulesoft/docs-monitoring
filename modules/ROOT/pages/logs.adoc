...
From Anypoint Monitoring, find *Log Search* to search for text in logs from the search field, through a filter, or with a DSL query.
...
image::log-filter-remove-field.png["Removing a filter column from the logs"]
...
image::log-filter-column-move.png["Removing a filter column from the logs"]
