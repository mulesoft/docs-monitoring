...
The _time period_ defines how long the metric meets the criteria defined using the operator and chosen value. Note that the a basic alert is triggered only if the condition is met continuously for the selected time period, not cumulatively. The following time periods are available:
