...
+
image::custom-metric-dc-select-target.png["A list of targets for a workflow"]
+
...
+
image::custom-metric-dc-add-dimension.png["A form with fields for metric name, dimensions, facts, and a checkbox for runtime exception on validation failure"]
...
.. Click *OK*. 
+
image::custom-metric-dc-add-item.png["A form with fields for dimension name, value, collapsible option and OK button"]
+
...
+
image::custom-metrics-add-fact.png["A form with fields for a fact name, a value, and an OK button"]
+
.. Click *OK*. +
You can repeat these steps to add multiple key values.
...
.. Click the *Custom Metrics Connector* to open the connector on Anypoint Exchange.
.. Click *Dependency Snippet*, and copy the Maven snippet.
. In Anypoint Studio, add the snippet for the connector to your project's `pom.xml` file. 
+
...
+
image::custom-metrics-connector-config.png["Send Custom Metrics source general configuration properties form with example values for contextual information and measurements"]
+
