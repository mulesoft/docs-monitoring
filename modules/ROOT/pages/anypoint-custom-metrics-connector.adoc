...
NOTE: To ensure optimal performance, Anypoint Monitoring groups metrics based on unique combinations of your custom dimensions and supports up to 100 unique combinations. If your data exceeds this limit, lower-volume combinations are grouped under a single *Other* category. This limit applies to all custom dimensions combined. For more information, see xref:app-dashboards.adoc#endpoint-operation-metrics[Endpoint and Operation Metrics Collection (Other Group)].
...
[IMPORTANT]
<<<<<<< HEAD
To use a custom metric, you must add it to a chart within two days of creating the metric. After two days, the name no longer appears in the list of metrics available as a *metric_name* value in the advanced chart configuration UI. Custom metrics appear in the dropdown after an app produces logs on the custom metric. 
=======
To use a custom metric, you must add it to a chart within two days of creating the metric. After two days, the name no longer appears in the list of metrics available as a *metric_name* value. Custom metric names appear in the configuration dropdown only after an app produces logs on the custom metric.
>>>>>>> 2d93659 (W-226911422-metric-names-duke)
...
** In the *Custom Metrics* tab of the built-in dashboard for an application, fact names defined in one CloudHub application are visible to other applications. 
+
