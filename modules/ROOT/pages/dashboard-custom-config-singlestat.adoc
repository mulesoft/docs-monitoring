...
Configure singlestat charts for Mule app and APIs in an Anypoint Monitoring custom dashboard. A singlestat charts displays a single value, such as a JVM - CPU Load Average value.

[[prereqs]]
== Before You Begin

Create or navigate to the singlestat chart you plan to configure:

* If your singlestat exists, open its configuration view:

. From *Custom Dashboards*, open the custom dashboard that contains your singlestat.
+
...
* If the singlestat doesn't exist:

. Create the singlestat.
+
