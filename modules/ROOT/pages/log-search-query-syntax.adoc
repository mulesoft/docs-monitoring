...
When performing a search, either specify a field name or use the default field. The field name and default field are specific to the search implementation.

Search any field by typing the field name, followed by a colon, then the term you are looking for.
As an example, assume a log search query language index contains two fields&#8212;application and message&#8212;and message is the default field. If you want to find the result called "The Right Way", which contains the text "don't go this way", enter:
