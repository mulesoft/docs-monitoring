...
In addition to using the query builder and facet search user interface in Anypoint Monitoring log search, you can use the log search query language.

== Search Terms

A search query consists of terms and operators. There are two types of search terms--single terms and phrases. A single term is one word, for example "test" or "hello". A phrase is a group of words surrounded by double quotes, for example "hello dolly".
...
To perform a multiple character wildcard search use the `*` symbol. Multiple character wildcard searches look for 0 or more characters. For example, to search for test, tests, or tester, you can use the search:
...
|`NOT`| The `NOT` operator excludes documents that contain the term after `NOT`. This is equivalent to a difference using sets. The symbol `!` can be used in place of the word `NOT`.|To search for documents that contain "anypoint monitoring" but not "results", use the query:
`"anypoint monitoring" NOT "results"` +
[NOTE]
...
When performing a search you can either specify a field name or use the default field. The field name and default field are specific to the search implementation.

You can search any field by typing the field name followed by a colon ":" and then the term you are looking for.
As an example, let's assume a log search query language index contains two fields--application and message--and message is the default field. If you want to find the result called "The Right Way" which contains the text "don't go this way", you can enter:
