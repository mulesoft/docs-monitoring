...
The agent's Omni Gateway ingress supports request counts. The charts reflect calls to the agent through that ingress.
...
MCP servers protected by an Omni Gateway ingress policy (xref:gateway-home::index.adoc#policy-availability-by-gateway[MCP Support]) produce metrics in these charts:
...
The MCP server's Omni Gateway ingress supports request counts. The charts reflect calls to the MCP server through that ingress.
