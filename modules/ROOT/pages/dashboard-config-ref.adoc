...
:description: Reference for predefined values, identifiers, and configuration options for custom dashboards in MuleSoft Anypoint Monitoring.
:keywords: MuleSoft, Anypoint Monitoring, custom dashboards, dashboard configuration, dashboard reference, metrics configuration, advanced mode
...
//RUNTIME_MEASUREMENTS: "app_inbound_metric,app_outbound_metric,app_stats,disk_stats,eden_space,jvm.classloading,jvm.cpu.operatingsystem,jvm.garbagecollector.copy,jvm.garbagecollector.marksweepcompact,jvm.garbagecollector.parnew,jvm.memory,jvm.memory.code_cache,jvm.memory.compressed_class_space,jvm.memory.metaspace,jvm.memory.par_eden_space,jvm.memory.par_survivor_space,jvm.memory.tenured_gen,jvm.runtime,jvm.threading,operatingsystem,survivor_space,custom_metric"
//ANALYTICS_MEASUREMENTS: "http_summary_metric"
...
* `avg_request_count`
* `avg_response_count`
| Application Statistics | `app_stats` a|
...
* `disk_used`
* `disk_free`
//TODO: NO Select options in UI
| Eden Space | `eden_space` a|
...
//TODO: IS THIS ONE/COPY FOR REAL?
//| JVM Garbage Collector Copy | `jvm.garbagecollector.copy` a|
//
//N/A
...
* `max_memory`
* `used_memory`
| JVM Memory Metaspace | `jvm.memory.metaspace` a|

* `committed_memory`
* `init_memory`
* `max_memory`
* `used_memory`
| JVM Memory (Par Eden Space) | `jvm.memory.par_eden_space` a|
...
* `cpu_steal_time`
* `cpu_io_wait`
//REMOVED FROM HYPERFORCE EU/US MAY 2026 -- WAS DEPRECATED IN PROD
// | HTTP Summary +
//*NOTE:*
...
// * `response_time_min`
// * `response_time_sum`
// TODO: NO Select values for survivor_space
| Survivor Space | `survivor_space` a|
 
N/A
// TODO: Not clear if custom_metric Select values are custom
// | Custom Metric | `custom_metric` a|
// 
// N/A
|===
...
//HTTP_MEASUREMENTS: "api_summary_metric,api_client_ip_metric,api_path_metric"

//NOTE: These match STGX
