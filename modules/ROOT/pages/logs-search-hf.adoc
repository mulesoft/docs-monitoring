...
+
Without quotes, multi-word searches return matches to each word, and special characters can affect the search. For partial word searches, you can use pattern matching characters in your search, such as `myap*`, or `appId: mya?p`.
...
[calloutlist]
. To expand log entry and view log fields, click *&gt;* in the *Time* field for a log entry. 
+
...
* Provide a custom label for the filter.
+
