...
[calloutlist]
. To view the full log entry with all its fields, click *&gt;* in the *Time* field for a log entry. 
. The full log provides *Table* and *JSON* views. 
. For long, truncated log messages, you can use *Table* view to get full message by clicking the *&gt;* in table's *Message* field. 
. You can also use *View surrounding logs* to get context by displaying a configurable number of logs that preceded and followed the log, and you can use *View log* to open the log entry in a dedicated page.
