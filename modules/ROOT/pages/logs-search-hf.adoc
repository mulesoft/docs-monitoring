...
Create, edit, and apply log search filters. When defining a filter, you can use selectable fields and operators, or you can use the JSON-based OpenSearch Query Domain-Specific Language (Query DSL) for more complex filters. To save a filter for future use, see <<save-search>>.
...
* *Invert inclusion* to convert filters that use `NOT` to exclude matching results into filters that include matching results, and to include filters that include matching results into filters that use `NOT` exclude matching results
* *Invert enabled/disabled* to disable all enabled filters and enable all disabled filters
// * *Pin all*
// * *Unpin all*
