...
[.notice-banner]
From May through June 2026, Anypoint Monitoring is rolling out an enhanced Log Search experience on the Hyperforce infrastructure in the US and EU instances of Anypoint Platform. Over time, this experience will replace the xref:monitoring::logs-us-eu.adoc[traditional Log Search experience]. When your Log Search experience changes, you can use this Hyperforce version of the Log Search documentation.
...
* For agentic log searches, see <<search-agentic>>.
* For on-premises servers with customized logging, see important information in <<caution-custom-logs>>.
====
...
You can use OpenSearch Query Domain-Specific Language (Query DSL) to create a filter. To learn the basics of Query DSL, see https://docs.opensearch.org/3.3/query-dsl/[Query DSL] in the v3.3 OpenSearch documentation. 
...
For more information, see <<field-names, valid field identifiers to use in Logs Search>> when creating a filter with Query DSL.
...
+
You can also negate the top-5 value filter by clicking the *-*. For example, you might use these features to create a filter on a log-level value (*log-level: ERROR*) or negate the value (*NOT log-level: ERROR*).
