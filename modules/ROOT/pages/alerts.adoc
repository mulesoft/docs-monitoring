...
You can implement any of several types of alerts. From the *Alerts* page in Anypoint Monitoring, you can create xref:basic-alerts.adoc[basic alerts], or you can link to alerts for API Manager and Runtime Manager alerts.
...
Using Anypoing Monitoring, you can configure two different types of alerts:
...
When you set up alerts, you can enable or disable any of them, filter alerts based on the <<alert_status, alert status>>, filter alerts based on the <<alert_severity, alert severity>>, and link to the graph in a custom dashboard that is associated with the alert.
...
All alerts have a state depending on
...
include::partial$include-alerts-state.adoc
