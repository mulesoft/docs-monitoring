...
- If you encounter inconsistent path normalization, refer to the <<normalize-path, Normalize API Paths>> section.
...
You can configure this property when deploying a new application to CloudHub or on a running Mule application in Runtime Manager: 
...
By defining an explicit pattern, you can ensure consistent normalization of dynamic endpoints in your analytics data. All incoming requests matching the pattern are correctly normalized to a single endpoint.

For example, provide a pattern that identifies the variable part:

Pattern: `/api/V1/proc/payments/{}/cancel` +
Where `{}` represents the variable part.

Incoming request: `/api/V1/proc/payments/CAjjAXjh/cancel` +
Where `CAjjAXjh` is the variable part `{}` in the incoming request.

Normalized result: `/api/V1/proc/payments/X/cancel` +
Where `X` is the normalized variable part `CAjjAXjh` in your analytics.
