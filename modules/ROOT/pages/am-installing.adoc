...
* To change the telemetry listening port (defaults to port `8888`) or provide multiple ports on a Linux host, see <<port-change>>. 

[[port-change]]
=== Set Listening Ports

When you install an agent instance, you can set the telemetry listening port. The default port is 8888.

To use multiple ports on a single Linux host, install separate agent instances as services. Each instance must have a unique installation name and a free port.

* To set a custom port, use the `-t` parameter. Example: `$ ./install -t 8889`
* To configure multiple ports on a single Linux host, assign a unique port and installation name to each agent instance. The agent must be installed as a service.
