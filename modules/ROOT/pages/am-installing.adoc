...
If you have an outbound firewall, open one of these ports based on the `am.zip` version:

* 443 for all agent versions 
* 8443 for agent versions 3.x and later
...
* Ubuntu 20.04
* Windows Server 2019 (not currently available for the 3.x agent)
...
In the Mule runtime engine on your server, install a supported version of runtime-manager::runtime-manager-agent.adoc[Anypoint Runtime Manager agent].

Anypoint Monitoring agent version 3.x and higher requires Mule Runtime Manager Agent version 2.7.1 or higher. See xref:.
...
You must manage Mule deployments in Runtime Manager. To ensure they appear, use the Runtime Manager user interface or the ARM REST API for new deployments.
...
. Follow the steps in xref:monitoring-settings-page.adoc[] to download the agent's `am.zip` file and install the agent from *Settings*, in Anypoint Monitoring.
...
If you proxy your monitoring data as it leaves the data center, you must configure the Anypoint Monitoring agent to communicate through the proxy. The configuration and the supported protocol depends on the version of the agent:

* <<v3, Version 3.x and later>>: Supports the HTTP/HTTPS protocol and does not support SOCKS5 
* <<v2, Version 2.x>>: Supports the SOCKS5 protocol and does not support HTTP/HTTPS

[[v3]]
=== Configure Proxy Communication for a 3.x Agent (HTTP/HTTPS Protocol)

If you proxy your monitoring data as it leaves the data center and you're using a 3.x version on a <<os-support, supported operating system>>, the proxy must support the HTTP/HTTPS protocol, and you must configure the Anypoint Monitoring agent to communicate through the proxy.
...
[[v3]]
=== Configure Proxy Communication for a 2.x Agent (SOCKS5 Protocol)
