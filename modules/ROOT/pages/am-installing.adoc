...
If you proxy your monitoring data as it leaves the data center, you must configure an Anypoint Monitoring agent for the proxy (using `-p`), and your proxy must be deployed to SOCKS5-compatible server. The agent uses a protocol that requires SOCKS5. 
