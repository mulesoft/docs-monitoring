...
If you have an outbound firewall, open port 443 and 8443.

Outbound connections are made to the following endpoints:

* US Control Plane
** us1.ingest.mulesoft.com:8443 (only for am.zip versions 3.x and newer)
** data-authenticator.anypoint.mulesoft.com:443
** dias-ingestor-router.us-east-1.prod.cloudhub.io:443 (only for am.zip versions 2.x and older)
...
* EU Control Plane
** eu1.ingest.mulesoft.com:8443 (only for am.zip versions 3.x and newer)
** data-authenticator.eu1.anypoint.mulesoft.com:443
** dias-ingestor-router.eu-central-1.prod-eu.msap.io:443 (only for am.zip versions 2.x and older)
...
[NOTE]
The new Anypoint Monitoring Agent version 3.x and higher require Mule Runtime Manager Agent version 2.7.x or higher.
...
For am.zip versions 3.x and newer:

The latest version of the Anypoint Monitoring Agent now supports HTTP/S protocol proxy on Linux servers (only).
If you proxy your monitoring data as it leaves the data center, the proxy must support the HTTP/S protocol,
and you must configure the Anypoint Monitoring agent to communicate through the proxy.

To configure the agent for the HTTP/S proxy:
...
Below section is for am.zip versions 2.x and older:
...
. Open the log file for the data-transfer agent. For Anypoint Monitoring Agent versions 2.x and older,
the log file is in this path: `./am/filebeat/logs`. For Anypoint Monitoring Agent versions 3.x and newer, Linux only,
the log file is in the `./am/logs` path
[NOTE]
For Anypoint Monitoring versions 3.x and newer, if you are using Linux and the Anypoint Monitoring agent is running as a service, the log file is in the `/var/log/otel-collector` directory.
For Anypoint Monitoring versions 2.x and older, if you are using Linux and the Anypoint Monitoring agent is running as a service, the log file is in the `/var/log/filebeat` directory.
...
- If you find connection failure or retry entries, ensure that port 8443 and 443 in your outbound firewall are open.
