...
Mule deployments must be visible and managed using the Runtime Manager user interface. To ensure these appear in Runtime Manager, use the Runtime Manager user interface or the ARM REST API for new deployments.
...
If you have a Titanium subscription and a hybrid runtime server, you can disable log forwarding to Anypoint Monitoring if you have AM installer v.2.5.10 or later. To disable this feature, you must configure a JVM property on each hybrid server for which you want to disable log forwarding. You cannot disable log forwarding per application.
