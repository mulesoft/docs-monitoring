...
. Under *New Value*, add the pattern for your dynamic endpoint, for example: `/api/V1/proc/payments/{X}/cancel`.
. Click *Apply Changes*.
...
All incoming requests with an empty string or with a string value:

* `/api/V1/proc/payments//cancel` +
* `/api/V1/proc/payments/CAjjAXjh/cancel` +

Normalizes the result as: `/api/V1/proc/payments/X/cancel` +
