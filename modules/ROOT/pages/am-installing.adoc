...
* When you install the Anypoint Monitoring agent on a hybrid server in a server group, restart the server to enable Anypoint Monitoring to begin collecting metrics.
...
If you have a Titanium subscription and a hybrid runtime server, you can disable log forwarding to Anypoint Monitoring. You must have AM installer v.2.5.10 or later to disable this feature using a JVM property. You must configure this property manually on each hybrid server for which you want to disable log forwarding, and you cannot disable log forwarding per application.
