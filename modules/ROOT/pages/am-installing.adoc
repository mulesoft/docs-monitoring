...
[NOTE]
If you configure the server you are installing the Anypoint Monitoring agent on for a server group or cluster, then you must add the server to the server group or cluster before you install the Anypoint Monitoring agent. This is because the Anypoint Monitoring agent uses the server ID to publish metrics to the system and the server ID changes once you add the server to a server group or cluster.
