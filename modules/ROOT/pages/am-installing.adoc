...
If you have an outbound firewall, open ports based on the `am.zip` version:

* 443 for all agent versions 
* 8443 for 3.x agent versions
...
* Ubuntu 20.04
* Windows Server 2019 (not currently supported for the 3.x agent)
...
* 2.7.x
+
Anypoint Monitoring agent version 3.x and higher requires Mule Runtime Manager agent version 2.7.1 or higher. 
* 2.6.x
* 2.5.x
* 2.4.x
* 2.3.0
...
The configuration and supported protocol depends on the version of the agent:

* <<v3, Version 3.x>>: Supports the HTTP/HTTPS protocol and does not support SOCKS5 
* <<v2, Version 2.x>>: Supports the SOCKS5 protocol and does not support HTTP/HTTPS

See also, <<os-support>>.

[[v3]]
=== Configure Proxy Communication for 3.x Monitoring Agents (HTTP/HTTPS Protocol)

If you proxy your monitoring data as it leaves the data center and you're using a 3.x version, the proxy must support the HTTP/HTTPS protocol, and you must configure the Anypoint Monitoring agent to communicate through the proxy.
...
[[v3]]
=== Configure Proxy Communication for 2.x Monitoring Agents (SOCKS5 Protocol)

If you proxy your monitoring data as it leaves the data center and you're using a 2.x version of the Anypoint Monitoring agent, the proxy must support the SOCKS5 protocol, and you must configure the Anypoint Monitoring agent to communicate through the proxy.
...
- If you find connection failure or retry entries, ensure that port 8443 (agent version 3.x and later) and 443 (all agent versions) in your outbound firewall are open.
