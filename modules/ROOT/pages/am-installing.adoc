...
Install the Anypoint Monitoring agent on an on-premises server to monitor applications that are running on that server and managed in the cloud. For more information, see xref:mule-runtime::mule-standalone.adoc[].
...
. Meet the runtime prerequisites in <<prereqs>>:
+
