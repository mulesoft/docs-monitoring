...
* Japan Control Plane (3.x installers, version 3.2.18.11 or later)
** jp1.ingest.mulesoft.com:8443
** data-authenticator.anypoint.mulesoft.com:443
** s3.jp1-central-1.amazonaws.com:443

* Canada Control Plane (3.x installers, version 3.2.18.11 or later)
** ingest.ca1.mulesoft.com:8443
** data-authenticator.anypoint.mulesoft.com:443
//VERIFY FULL ENDPOINT:
** s3.ca1-central-1.amazonaws.com:443

* India Control Plane (3.x installers, version 3.2.18.11 or later)
** ingest.in1.mulesoft.com:8443
** data-authenticator.anypoint.mulesoft.com:443
** s3.in1-central-1.amazonaws.com:443

* Australia Control Plane (3.x installers, version 3.2.18.11 or later)
** ingest.au1.platform.mulesoft.com:8443
** data-authenticator.anypoint.mulesoft.com:443
//VERIFY FULL ENDPOINT:
** s3.au1-central-1.amazonaws.com:443
...
////
HIDING TEMPORARITY IN CASE THESE DON'T GO OUT
CHECK ALL CANADA PLS:
|*Canada*|ingest.ca1.platform.mulesoft.com | 8443
|*Canada*|ca1.anypoint.mulesoft.com | 443
|*Canada*|runtime-manager.ca1.platform.mulesoft.com | 8443
|*Canada*|analytics-ingest.ca1.anypoint.mulesoft.com |  443
|*Canada*|data-authenticator.ca1.anypoint.mulesoft.com |  443
|*Canada*|exchange2-asset-manager-kprod-ca1.s3.ca1-central-1.amazonaws.com |  443
CHECK ALL JAPAN PLS:
|*Japan*|ingest.jp1.platform.mulesoft.com | 8443
|*Japan*|jp1.anypoint.mulesoft.com | 443
|*Japan*|runtime-manager.jp1.platform.mulesoft.com | 8443
|*Japan*|analytics-ingest.jp1.anypoint.mulesoft.com |  443
|*Japan*|data-authenticator.jp1.anypoint.mulesoft.com |  443
|*Japan*|exchange2-asset-manager-kprod-jp1.s3.jp1-central-1.amazonaws.com |  443
CHECK ALL INDIA PLS:
|*India*|ingest.in1.platform.mulesoft.com | 8443
|*India*|in1.anypoint.mulesoft.com | 443
|*India*|runtime-manager.in1.platform.mulesoft.com | 8443
|*India*|analytics-ingest.in1.anypoint.mulesoft.com |  443
|*India*|data-authenticator.in1.anypoint.mulesoft.com |  443
|*India*|exchange2-asset-manager-kprod-in1.s3.in1-central-1.amazonaws.com |  443
////
|*Australia*|ingest.au1.platform.mulesoft.com | 8443
|*Australia*|au1.anypoint.mulesoft.com | 443
|*Australia*|runtime-manager.au1.platform.mulesoft.com | 443
|*Australia*|analytics-ingest.au1.anypoint.mulesoft.com |  443
|*Australia*|data-authenticator.au1.anypoint.mulesoft.com |  443
|*Australia*|exchange2-asset-manager-kprod-au1.s3.au1-central-1.amazonaws.com |  443
|===s
...
//ARE THERE ANY NEWER VERSIONS TO LIST HERE BEYOND 2.7.x? 
//Looks like versions go up to 2.8.2 as of 7/31/26: https://docs.mulesoft.com/release-notes/runtime-manager-agent/runtime-manager-agent-release-notes
* 2.7.x
+
...
You must manage Mule deployments in Runtime Manager. You can add and check for your hybrid deployments from the xref:runtime-manager::deploying-to-your-own-servers.adoc[Runtime Manager UI] or the xref:runtime-manager::runtime-manager-api.adoc[Runtime Manager REST API].
...
//follow am installer steps in hybrid setup page
include::partial$include-am-setup.adoc[tag="am-installer-setup"]
...
* To change the telemetry listening port (defaults to port `8888`), set the `-t` parameter to an integer from 1024 through 65535. 
+
Example: `$ ./install -t 8889`. 
* To install multiple instances of the agent on a Linux host, see <<install-multiple-agents>>. 
* For traces in Anypoint Monitoring, see <<tracing-setup>>.

[[install-multiple-agents]]
=== Install Multiple Agents to a Linux Host

On a Linux host, you can install multiple agent instances:

* Non-service installations: To install multiple agent instances, assign a unique port to each agent instance.
+
Set the `-t` parameter to an integer from 1024 through 65535.
+
Example: `$ ./install -t 8889`

* Service installations: To install multiple agent instances as services, assign a unique port and installation name to each instance.

** For the first service installation, you can use the default configuration. The `-t` and `-n` parameters are optional.
** For subsequent service installations, you must use both `-t` and `-n`. 
+
Example: `$ ./install -t <port> -n <installation-name>`
+
The installer prompts you for a unique port and installation name.

Parameters:

* `-t`: Sets the port number to an integer from 1024 through 65535. The default is 8888. 
* `-n`: Sets a unique name that becomes part of the service name. The name must be no more than eight alphanumeric characters. The default is an unnamed port. Required for the second and subsequent agent instances on a single Linux host. Supported with `-t` only for service installations.

[[tracing-setup]]
== Enable Trace Data Collection for Hybrid Runtime Servers

//feature tier (for h2-h4 headings):
include::partial$subsection-pills.adoc[tag="anypoint-pricing-titanium"]

For installer version 3.2.14 or later, you can enable trace data collection for Anypoint Monitoring. On each hybrid server where you want to collect trace data, configure a JVM property and install agent with the trace data collection option. Version 3.2.1.4 was released on xref:release-notes::monitoring/anypoint-monitoring-installer-release-notes.adoc#march-5-2026[March 5, 2025].

. Upgrade to an Anypoint Monitoring agent version that supports trace data collection (am-3.2.14.0.zip or later).
. Stop Mule.
. Add this line to `$MULE_HOME/conf/wrapper.conf`:
+
`wrapper.java.additional.<N>=-Dmule.openTelemetry.tracer.exporter.enabled=true`
+
`<N>`: The next available property index.
. Install the agent using the `-g 4317` option:
+
`$MULE_HOME/am/bin/install -g 4317`
+
Mule runtime uses port 4317 for trace data collection by default (see xref:mule-runtime::otel-support.adoc[]).
. Restart Mule.
. Finish setting up the agent:
+
`$MULE_HOME/am/bin/setup`
...
[NOTE]
If necessary, you can add the endpoint to an allowlist for outbound firewall rules so your server allows connections. The endpoints to add to the allowlist are listed in <<prereqs>>.
...
- If you encounter inconsistent path normalization, refer to <<normalize-path>>.
...
//Hybrid, CH2, and CH
include::partial$include-path-normalization.adoc[]

[[tier-change]]
== Restart After Changing Subscription Tiers

For the _2.x version_ of Anypoint Monitoring agent, changes to your subscription tier require further action to continue collecting metrics. 
