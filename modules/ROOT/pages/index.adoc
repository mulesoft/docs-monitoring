...
Search for raw log and event data from across the app network so that you can pinpoint the root cause of a problem.
...
Log search is also available from Runtime Manager.  tier provides a link to Log Search from Anypoint Monitoring. For the Starter tier, you can navigate to Runtime Manager, and open *Logs*. 
