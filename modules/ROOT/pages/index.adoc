...
NOTE: Anypoint Monitoring provides application-level metrics for Mule apps and APIs. VPC-level network traffic and monthly outbound byte totals are not available. For details on metrics scope and how to estimate traffic using the Metrics API, see xref:am-faq.adoc#vpc-traffic-metrics[Anypoint Monitoring FAQ].
...
| xref:dashboards.adoc[Built-in dashboards (Classic mode)]
| Yes
| No
| No
| No
