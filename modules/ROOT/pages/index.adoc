...
Anypoint Monitoring is the standard method of monitoring Mule application performance, logging, and troubleshooting to reduce the mean time to identify issues and resolve them. While all customers receive fundamental monitoring capabilities with their subscription, the Titanium tier offers greater storage, guaranteed resources, and advanced capabilities that allow for greater control and flexibility.

|===
| Feature | Gold/Platinum Support | Titanium Support

3+| *Built-in Application Dashboards*
| More than 80 out-of-the-box metrics for Overview, Inbound, Outbound, Performance, Failures, JVM, and Infrastructure charts
| Yes
| Yes
