...
:description: Use Anypoint Insights in MuleSoft Anypoint Monitoring to view standard, predefined performance metrics for your Mule apps and API implementations across all environments.
:keywords: MuleSoft, Anypoint Monitoring, Anypoint Insights, app metrics, API metrics, performance metrics, standard dashboards, predefined dashboards, Mule app monitoring
...
To find the equivalent charts in Anypoint Insights, see <<dashboard-chart-mapping, Chart Mapping from Built-in Dashboards>>.
...
NOTE: Queries can return empty data (`200 OK` with no results) if the Connected App lacks the required environment-level permissions.
Also, the `endpoint.type` and `endpoint.operation` attributes are no longer valid on the `mulesoft.api.summary` metric.
...
[[dashboard-chart-mapping]]
== Chart Mapping from Built-in Dashboards

If you previously used built-in dashboards in Anypoint Monitoring, use these tables to find the equivalent charts in Anypoint Insights.

=== Mule App Charts

include::partial$include-insights-parity-charts.adoc[tags=app-chart-parity]

=== API Charts

include::partial$include-insights-parity-charts.adoc[tags=api-chart-parity]
