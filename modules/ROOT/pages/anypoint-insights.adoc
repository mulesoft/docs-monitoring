...
. <<filters, Filters>> to narrow the data in charts
. Performance <<charts-performance, metrics and trends>> across your deployed Mule apps and APIs
. <<entities-list, List of each Mule app and API>> that you can click to display detailed metrics on a specific <<entity-metrics-apps, app>> or <<entity-metrics-apis, API>>
...
Anypoint Insights uses <<metrics-api, Anypoint Monitoring Metrics API>> to query for data that appears in charts. Data aggregation and retention limits in Anypoint Insights are derived from the service that supports the Metrics API. For information about data collection intervals in Anypoint Insights charts, see <<data-collection>>.

For Advanced and Titanium subscription tiers, Integration Intelligence is accessible from Anypoint Insights. See xref:integration-intelligence.adoc[] to extend your MuleSoft telemetry data analysis beyond real-time monitoring.
...
** https://in1.platform.mulesoft.com/login/[India Cloud]


* A Mule app or API (xref:gateway-home::index.adoc[Mule Gateway or Omni Gateway API]) running on a supported version of Mule runtime engine (see xref:release-notes::mule-runtime/mule-esb.adoc[]) and deployed to a supported target:
...
Anypoint Insights reports transaction-level and flow-level timing for inbound and outbound endpoints, flow start and completion, and custom business events. Metrics don't include a per-component, per-event-processor timing breakdown for every processor within a flow. For processor-level latency, use xref:traces.adoc[Traces] or xref:telemetry-exporter.adoc[Telemetry Exporter].
...
include::partial$include-insights-charts.adoc[tags="data-scope"] 
Only entities with traffic in the selected period and <<change-bg, business group>> appear in *Entities List*.
...
[[dashboard-details-app]]
//don't change order of numbers in graphic or calloutlist
image::insights-details-app.png["Anypoint Insights App Details UI"]
...
* <<entity-metrics-app-flows>>
* <<entity-metrics-app-connectors>>
* <<entity-metrics-app-jvm>>
* <<entity-metrics-app-custom-metrics>>
...
To compare *Application Details* charts and visualizations with those in the traditional  built-in dashboards for the same app, see xref:monitoring::app-dashboards.adoc#app-chart-parity[App Chart Mapping].
...
include::partial$include-insights-charts.adoc[tag="app-detail-flows"]

[[entity-metrics-app-connectors]]
==== Connector Metrics for a MuleSoft Connector
...
include::partial$include-insights-charts.adoc[tag="app-detail-connectors"]

[[entity-metrics-app-jvm]]
==== JVM Metrics for a Mule Application

//TODO: REMOVE IF/WHEN THIS NO LONGER APPLIES
//feature tier (for h2-h4 headings):
//include::partial$subsection-pills.adoc[tag="anypoint-cloud-support-jvm"]
...
[[entity-metrics-app-custom-metrics]]
==== Custom Metrics for a Mule Application
...
To create custom metrics, use the Anypoint Custom Metrics Connector in your app flow to collect metrics for your business and operational needs. After you deploy your app, your defined custom metrics charts appear in this tab. For guidance using the Anypoint Custom Metrics Connector, see xref:monitoring::anypoint-custom-metrics-connector.adoc[].

//TODO: WHAT DEPLOYMENT TARGETS SUPPORT CUSTOM METRICS? VERIFY AND ADD TO PREREQS IF NECESSARY (I SEE server or server cluster in built-in metrics doc but not clear about CH, CH2, RTF, etc.)

[[custom-metrics-chart-config]]
===== Configure Custom Metrics Charts
...
Chart settings correspond to and reflect values available to Mule apps from built-in dashboards (in *Built-in Dashboards*) that have custom metrics. The *Dimensions* setting is named *Group By* in built-in dashboards. See xref:anypoint-custom-metrics-connector.adoc#custom-metrics-chart-settings[Configure Charts with Custom Metrics in a Built-In Dashboard]).
...
//don't change order of numbers in graphic or calloutlist
image::insights-details-api.png["Anypoint Insights API Details UI"]
...
To compare *API Details* charts and visualizations with those in the traditional built-in dashboards for the same API, see  xref:monitoring::anypoint-insights.adoc#entity-metrics-apis[*API Details*] in Anypoint Insights.
...
[[entity-metrics-api-policies]]
==== Policy Metrics for an API Implementation 
...
include::partial$include-insights-charts.adoc[tags="api-detail-policies"]
...
//feature tier (for h2-h4 headings):
include::partial$subsection-pills.adoc[tag="anypoint-pricing-all-tiers"]
...
To use the API and get information about data retention, see the API documentation on Anypoint Exchange: https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/f1e97bc6-315a-4490-82a7-23abe036327a.anypoint-platform/metrics-api/[Anypoint Monitoring Metrics API]

NOTE: Two common issues when querying the Metrics API: queries can return empty data (200 OK with no results) if the Connected App lacks the required environment-level permissions, and certain attributes such as `endpoint.type` and `endpoint.operation` are no longer valid on the `mulesoft.api.summary` metric.
For details and workarounds, see xref:am-faq.adoc#metrics-api-empty-data[Why does my Metrics API query return empty data?] and xref:am-faq.adoc#metrics-api-traffic-attributes[What attributes should I use to query application traffic data?]
