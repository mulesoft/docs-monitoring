...
* A Mule app or API (xref:gateway-home::index.adoc[Mule Gateway or Omni Gateway (formerly Flex Gateway) API]) running on a supported version of Mule runtime engine (see xref:release-notes::mule-runtime/mule-esb.adoc[]) and deployed to a supported target:
