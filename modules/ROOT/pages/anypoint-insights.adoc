...
.. Click the its name in *Entities List*.
...
Use one or more filters to narrow the data in your charts. The filters support operators `=` and `IN` (for any matches to a list of items). Multiple filters are allowed. For example, `entity_type = 'APP' deployment_type = 'CLOUDHUB2'` returns all Mule apps deployed to CloudHub 2.0.

[%header,cols="10a,40a,10a,40a"]
|===
| Filter | Description | Operators | Example

|`deployment_type`
|Filter by deployment target, CloudHub 2.0 (*CLOUDHUB2*) or Runtime Fabric (*RTF*).
|`=`, `IN`
|`deployment_type IN ['CLOUDHUB2', 'RTF']`

|`entity_type`
|Filter by deployment type (*APP* or *API*).
|`=`, `IN`
|`entity_type = 'APP'`

|`entity`
|Filter by specific app or API deployments. 
|`=`, `IN`
|`entity IN ['99aea888-77de-6e06-5c55-d1234de56789', '09f99999-888d-7b77-6da5-bebd43210fc1']`

|`entity_response_status`
|Filter by the response code (*OK* or *FAILED*).
|`=`
...
If you don't see data in any of the cards, charts, or tables, ensure that you have chosen the appropriate time range in the date picker.
...
.. In Access Management *Settings*, select an environment as the default deployment environment to view for the selected business group in Anypoint Monitoring.
+
image::insights-environment-selector.png["Environment selector in Access Management"]
.. Return to Anypoint Monitoring to view the new business group.
+
...
.. Click your profile icon, and select the name of your profile.
.. In Access Management *Settings*, select a business group to use as the default.
.. Return to Anypoint Monitoring to view the new business group.
+
Refresh the page, if necessary. 
